    ...
    for di in d:
        ...
        for z in _matlab_range(zmin, dz, zmax):
            for x in _matlab_range(xmin_const, dx, xmax_const):
                for y in _matlab_range(ymin_z(z), dy, ymax_z(z)):
                    total += fun(x, y, z) * dx * dy * dz
        I3.append(total)
        ...
        total = _riemann_3d(total, zmin, zmax, dz, xmin4, xmax4, dx, ymin4, ymax4, dy, fun)
        I4.append(total)
...
def _riemann_3d(
    ...
    for z in _matlab_range(zmin, dz, zmax):
        xl = xmin(z)
        xu = xmax(z)
        for x in _matlab_range(xl, dx, xu):
            yl = ymin(x, z)
            yu = ymax(x, z)
            for y in _matlab_range(yl, dy, yu):
                total += fun(x, y, z) * dx * dy * dz
    return total
